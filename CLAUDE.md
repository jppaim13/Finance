...
- `apps-script.gs` — legacy backend: a Google Apps Script that reads/writes a Google Sheet, deployed as a Web App (`doGet`/`doPost` action-dispatch pattern). **Unreachable since the Fase 0 auth gate**: `showLoginScreen()` always requires a Supabase client + session before `#appRoot` becomes visible, and there is no code path into the app that doesn't go through it — the file is still in the repo but nothing in `index.html` can currently reach it. Not deleted yet; a decision the user should make explicitly, not something to "restore" reachability for by loosening the gate.
...
**Supabase** is the only reachable backend: the client (`_sb`, created by `conectarSupabase()`) talks directly to Postgres tables via `sbSelect`/`sbInsert`/`sbUpdate`/`sbDelete` helpers — thin wrappers around `supabase-js`. `apps-script.gs` (Google Sheets backend, `doGet`/`doPost` dispatch on an `action` param) is legacy and currently **unreachable** — see the file list above.
...
- **`date-utils.js` versionado via query string (`?v=N`) no `<script src>`, sincronizado com `SHELL` em `sw.js`.** O Service Worker só é network-first pro `index.html`; todo o resto (`date-utils.js` incluso) é cache-first. Sem versionamento, dava pra existir uma janela com `index.html` novo + `date-utils.js` velho em cache, sem erro nenhum — o sintoma seria saldo/data errados, não uma exceção. Bump o `N` em **ambos os lugares** (`<script src="date-utils.js?v=N">` no `index.html` e a entrada correspondente em `SHELL`) sempre que o conteúdo de `date-utils.js` mudar; bump `CACHE_NAME` em `sw.js` junto, pra forçar a invalidação do cache antigo. Mudanças só em `index.html` não precisam bump nenhum.
